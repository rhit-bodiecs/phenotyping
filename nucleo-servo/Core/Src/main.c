...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void servo_write_us(uint16_t pulse_us)
{
    /*
     * Keep the pulse within a conservative initial range.
     * Expand this only after confirming the servo does not hit its stops.
     */		
...
{	
	//jitter for about one second
	for(int i = 0; i < JITTER_COUNT; i++){
		servo_write_us(SERVO_START + SERVO_JITTER_US);
		HAL_Delay(JITTER_DELAY);

		servo_write_us(SERVO_START - SERVO_JITTER_US);
		HAL_Delay(JITTER_DELAY);
	}
	
	servo_write_us(SERVO_START);
	HAL_Delay(2000);
	servo_write_us(SERVO_DUMP);
	HAL_Delay(2000);
	servo_write_us(SERVO_START);
